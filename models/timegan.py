...
class TimeGAN(torch.nn.Module):
    ...
    def _discriminator_forward(self, X, T, Z, gamma=1):
        ...
        #gp = self.gradient_penalty(self.discriminator, X, E_hat, T)
        ...
        return D_loss
    def gradient_penalty(self,dis, real_data, fake_data, T):
        alpha = torch.rand(real_data.size(0), 1, 1).cuda().expand_as(real_data)
        interpolated = Variable(alpha * real_data + ((1 - alpha) * fake_data),
                                requires_grad=True)
        with torch.backends.cudnn.flags(enabled=False):
            interpolated_scores = dis(interpolated,T)
        gradients = torch.autograd.grad(
            outputs=interpolated_scores,
            inputs=interpolated,
            grad_outputs=torch.ones(interpolated_scores.size()).cuda(),
            create_graph=True,
            retain_graph=True,
            only_inputs=True
        )[0]
